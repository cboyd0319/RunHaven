use super::instructions::{
    Action, BYTE_LENGTH, DebugVerbosity, InputStreamEnum, MemoryReader, ObjectTreeReader,
    OutputStreamEnum, Property, WORD_LENGTH, ZCodeVersion, ZMACHINE_FALSE, ZMACHINE_TRUE,
    ZmachineError, handle_instruction, word_to_signed,
};
use super::interfaces::{QuetzalData, TerpIO};
use super::quetzal::iff::QueztalStackFrame;
use super::story::{
    A0_CHARS, A2_CHARS, ABBREV_1, ABBREV_2, ABBREV_3, DEFAULT_UNICODE_MAPPING, NOPRINT_CHAR,
    SHIFT_DOWN, SHIFT_LOCK_DOWN, StatusMode, TOGGLE_EXTENDED, ZCharacterMapper,
    ZCharacterMapperStub, ZCharacterMapperV2, ZCharacterMapperV3,
};
use std::cmp;
...
impl VM {
    ...
    fn print_to_output(&mut self, text: &str, io: &mut dyn TerpIO) {
        if self.memory_stream_stack.is_empty() {
            ...
            let _ = self.print_to_memory(len_addr, text);
        }
    }
    ...
    pub fn reseed_rng(&mut self) {
        self.rng = Box::new(RandomModeRNG::new());
    }
    ...
    // RunHaven keeps the vendored engine silent inside the full-screen TUI.
    fn debug(&self, message: String) {
        let _ = message;
    }
    // RunHaven surfaces engine failures through the owning UI state instead.
    ...
    pub fn set_transcript_bit(&mut self, b: bool) {
        let _ = self.set_bit(HEADER_FLAGS_2, 0, b);
    }
    ...
    pub fn dump_memory(&self, start_address: usize, end_address: usize, width: usize) {
        let _ = (self, start_address, end_address, width);
    }
