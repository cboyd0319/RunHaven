...
| --- | --- | --- |
| Binary entrypoints | `crates/runhaven/src/main.rs`, `crates/runhaven/src/bin/runhaven-check-pins.rs` | `runhaven` is binary-only. Do not recreate a root compatibility facade; shared behavior belongs in `runhaven-core`. |
...
| Test suite | crate-local `tests/` directories plus module tests | Focused Rust tests cover CLI, plans, egress, images, state, worktrees, auth, Tauri commands, and repo policy. |
