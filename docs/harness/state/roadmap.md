...
| Tauri/Svelte first scaffold | validated | RunHaven has a minimal desktop app shell that reads setup/dashboard/profile/run-plan state without widening the CLI security boundary | `ui/`, `src-tauri/`, exact pins, lockfiles, Tauri capabilities, typed read-only commands, frontend tests | Start only from `docs/TAURI_UI_RESEARCH_PLAN.md` and `docs/TAURI_UI_GUARDRAILS.md`; no mutating commands in the first scaffold | Maintainers | 2026-06-16: `./init.sh` passed after adding root Rust checks, `npm --prefix ui ci --ignore-scripts`, frontend typecheck/tests/build, Tauri Rust fmt/test/clippy, and `npm --prefix ui run tauri:build`. | Reopen when adding launch, stop, kill, attach, repair, image build, state reset, network prune, or worktree review controls |
