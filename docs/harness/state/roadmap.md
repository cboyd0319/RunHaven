...
| Tauri launch execution | validated | The desktop app can start a run only after plan review, launch confirmation, and warning acknowledgement | `src-tauri/src/commands/`, `src-tauri/capabilities/`, `src/runhaven/runtime/launch.rs`, `ui/src/`, docs, tests | Reuse existing Rust validators and runtime launch code; no shell/process bridge | Maintainers | 2026-06-16: focused Rust and frontend tests, Svelte check, Playwright launch preview, Tauri tests/clippy, root Rust tests, build, source-size guard, and `git diff --check` passed. | Reopen when adding image readiness, active-run resource warnings, live logs, stop, kill, attach, repair, image build, state cleanup, or worktree review controls |
