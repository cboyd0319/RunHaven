...
| Agent state volume | `runhaven` CLI | Per-project/profile/session state is locked during a run so concurrent agents cannot attach the same named volume. Named sessions reuse only the isolated `/home/agent` state volume and do not widen workspace mounts. `state reset` and session-filtered `state prune` delete only RunHaven-managed home volumes. |
