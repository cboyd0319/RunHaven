...
| Network calls | Project maintainers | Prefer local verification. Internet-enabled agent runs are unrestricted egress until provider allowlisting lands. The proxy pattern is smoke-proven, but reserved provider mode must fail closed until that lifecycle is integrated into normal runs. |
