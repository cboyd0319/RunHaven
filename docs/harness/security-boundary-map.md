...
| Network calls | Project maintainers | Prefer local verification. Default internet runs are unrestricted egress. Provider runs use a managed internal network plus host CONNECT proxy allowlist; extra hosts require explicit `--provider-host HOST`. |
