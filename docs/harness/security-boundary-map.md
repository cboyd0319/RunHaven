...
| Git worktrees | `runhaven` CLI | `run --worktree` requires a clean source git repository, creates a RunHaven-owned branch and worktree under the cache root, mounts that worktree for the agent, and keeps it for explicit user review. `runs keep`, `runs merge`, and `runs discard` validate the recorded RunHaven-owned worktree and branch before acting; merge refuses dirty or moved source checkouts, and discard removes only the recorded worktree and branch. |
