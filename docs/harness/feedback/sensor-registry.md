...
| `cargo run --locked --bin runhaven-check-pins` | `pins.toml`, Cargo manifests, image templates, workflow pins | Enforce exact package, image, runner, npm, Debian, and Action pin policy | Maintainers | Replace if pin policy moves into a stronger packaged command | Dependency, image, workflow, release, or docs changes that mention pins |
