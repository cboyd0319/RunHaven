...
| `capability_guard` test (`cargo test --manifest-path src-tauri/Cargo.toml`) | `src-tauri/capabilities/*.json` | Fail closed if a Tauri capability grants any scope beyond RunHaven `allow-*` command permissions plus a vetted plugin allowlist, so the untrusted WebView cannot gain a host bridge | Maintainers | Replace if the capability model or permission scheme changes | Tauri capability, command surface, or plugin permission changes |
