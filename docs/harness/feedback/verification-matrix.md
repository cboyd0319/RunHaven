...
| Apple `container` runtime boundary | Rust code checks plus `runhaven doctor`, `runhaven plan`, and a focused `runhaven run shell` smoke that proves the claimed mount, user, network, or filesystem behavior |
