...
| SSH forwarding boundary | Plan checks for `--ssh` command shape; run `scripts/apple_container_smoke.sh --with-ssh` when SSH forwarding behavior changes and the host/runtime are available |
