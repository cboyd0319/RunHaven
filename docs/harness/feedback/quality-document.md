...
| Provider egress and auth broker | B+ | Proxy, endpoint, auth broker, and Rust tests exist | Good diagnostics through `why host`, `egress log`, and `auth` commands | Broader path-sensitive hosts and non-Codex brokers need explicit design and evidence |
...
| --- | --- | --- | --- |
| Repo harness | Documented | Repo-owned docs checks plus advisory `harnessforge report --target .` and `harnessforge audit --target . --min-score 85` when available | Keep root instructions short and detail in `docs/harness/` |
| CLI and planner | Tested | Rust tests, clippy, pin checks | Command construction is security-sensitive |
...
| Provider proxy and auth broker | Tested plus selective live smokes | Egress/auth focused tests and manual provider-mode smokes | Keep logs secret-free and hosts explicit |
