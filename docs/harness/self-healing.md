...
3. Run `PYTHONPATH=../HarnessForge/src python3 -m harnessforge update --target . --apply` for missing safe artifacts only.
4. Run pin checks, tests, and `PYTHONPATH=../HarnessForge/src python3 -m harnessforge audit --target .`.
