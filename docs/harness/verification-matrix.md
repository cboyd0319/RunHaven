...
| Image and network repair UX | Python code checks plus focused `tests/test_cli.py` image rebuild and `tests/test_cli_network.py` managed-network list/prune tests; use `runhaven image rebuild --dry-run`, `runhaven network list`, and `runhaven network prune` smokes |
